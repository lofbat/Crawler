...
create table `question_type`(
	`typeid` int(11) not null primary key comment '问题类型编号',
	`type` text not null comment '问题类型'
) comment='问题类型表';
insert into question_type values(1,'定义型');
insert into question_type values(2,'原因型');
insert into question_type values(3,'列举型');
insert into question_type values(4,'方法型');
insert into question_type values(5,'其他型');

create table `knowledge_point`(
	`knowid` int(11) not null primary key comment '知识类型编号',
	`knowpoint` text not null comment '知识点类型'
) comment='知识点类型表';
insert into knowledge_point values(1,'作业管理');
insert into knowledge_point values(2,'进程管理');
insert into knowledge_point values(3,'存储管理');
insert into knowledge_point values(4,'文件管理');
insert into knowledge_point values(5,'设备管理');
insert into knowledge_point values(6,'系统概论');

create table `question_answer`(
	`questionid` int(11) not null primary key auto_increment comment '问题编号',
	`question` varchar(2000) not null comment '问题',
	`answer` text(8000) not null comment '答案', 
	`typeid` int(11) not null comment '问题类型编号',
	`knowid` int(11) not null comment '知识类型编号'
) comment='操作系统问题表';
insert into question_answer(question,answer,typeid,knowid) values('操作系统是什么','是操作系统',1,6);
insert into question_answer(question,answer,typeid,knowid) values('为什么能存储','所以能存储',2,3);
insert into question_answer(question,answer,typeid,knowid) values('作业有哪几部分','作业有好几部分',3,1);
insert into question_answer(question,answer,typeid,knowid) values('怎么独立线程','这么独立',4,2);